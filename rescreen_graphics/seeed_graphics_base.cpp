...
void line::draw() {
    tft.drawLine(_x0, _y0, _x1, _y1, _color);
}
void dash_line::draw() {
    point origin(_x, _y);
    point solid_offset;
...
void rectangle::draw() {
    ...
    tft.drawRect(left_top.x, left_top.y, _width, _height, _color);
    
    if (_fill != transparent) {
        tft.fillRect(left_top.x + 1, left_top.y + 1, _width - 2, _height - 2, _fill);
    }
}
...
void ellipse::draw() {
    ...
    if (_fill){
        tft.fillEllipse(_x, _y, _width / 2, _height / 2, _fill);
    }
    tft.drawEllipse(_x, _y, _width / 2, _height / 2, _color);
}
text & text::font(font_t value){
    _font = value;
    tft.setTextFont(uint8_t(value));
    return this[0];
}
