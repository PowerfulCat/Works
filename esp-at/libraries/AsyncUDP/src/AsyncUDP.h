...
class AsyncUDP;
class AsyncpacketBuffers;
class AsyncUDPMessage;
struct udp_pcb;
struct pbuf;
struct netif;

typedef std::function<void(AsyncpacketBuffers& packet)> AuPacketHandlerFunction;
typedef std::function<void(void * arg, AsyncpacketBuffers& packet)> AuPacketHandlerFunctionWithArg;
...
class AsyncpacketBuffers : public Stream
{
protected:
    AsyncUDP *_udp;
    pbuf *_pb;
    ...
    size_t _index;
public:
    AsyncpacketBuffers(AsyncUDP *udp, pbuf *pb, const ip_addr_t *addr, uint16_t port, struct netif * netif);
    virtual ~AsyncpacketBuffers();
