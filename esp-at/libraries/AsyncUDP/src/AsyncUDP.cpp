...
AsyncpacketBuffers::AsyncpacketBuffers(AsyncUDP *udp, pbuf *pb, const ip_addr_t *raddr, uint16_t rport, struct netif * ntif)
{
    _udp = udp;
    _pb = pb;
...
AsyncpacketBuffers::~AsyncpacketBuffers()
{
    pbuf_free(_pb);
}

uint8_t * AsyncpacketBuffers::data()
{
    return _data;
}

size_t AsyncpacketBuffers::length()
{
    return _len;
}

int AsyncpacketBuffers::available(){
    return _len - _index;
}

size_t AsyncpacketBuffers::read(uint8_t *data, size_t len){
    size_t i;
...
void AsyncpacketBuffers::flush(){
    _index = _len;
}

tcpip_adapter_if_t AsyncpacketBuffers::interface()
{
    return _if;
}

IPAddress AsyncpacketBuffers::localIP()
{
...
IPv6Address AsyncpacketBuffers::localIPv6()
{
...
uint16_t AsyncpacketBuffers::localPort()
{
    return _localPort;
}

IPAddress AsyncpacketBuffers::remoteIP()
{
    if(_remoteIp.type != IPADDR_TYPE_V4){
        return IPAddress();
    }
...
IPv6Address AsyncpacketBuffers::remoteIPv6()
{
...
uint16_t AsyncpacketBuffers::remotePort()
{
    return _remotePort;
}

void AsyncpacketBuffers::remoteMac(uint8_t * mac)
{
    memcpy(mac, _remoteMac, 6);
}

bool AsyncpacketBuffers::isIPv6()
{
    return _localIp.type == IPADDR_TYPE_V6;
}

bool AsyncpacketBuffers::isBroadcast()
{
    if(_localIp.type == IPADDR_TYPE_V6){
        return false;
    }
...
bool AsyncpacketBuffers::isMulticast()
{
    return ip_addr_ismulticast(&(_localIp));
}

size_t AsyncpacketBuffers::write(const uint8_t *data, size_t len)
{
    if(!data){
        return 0;
    }
...
size_t AsyncpacketBuffers::write(uint8_t data)
{
    return write(&data, 1);
}

size_t AsyncpacketBuffers::send(AsyncUDPMessage &message)
{
...
void AsyncUDP::_recv(udp_pcb *upcb, pbuf *pb, const ip_addr_t *addr, uint16_t port, struct netif * netif)
{
    while(pb != NULL) {
        ...
        this_pb->next = NULL;
        if(_handler) {
            AsyncpacketBuffers packet(this, this_pb, addr, port, netif);
            _handler(packet);
        } else {
            pbuf_free(this_pb);
        }
